...
const CardProductList = ({products}: {products: Product[]}) => {
  ...
  return (
    <motion.div initial={{opacity: 0, y: 50}} whileInView={{opacity: 1, y: 0}} transition={{duration: 0.6, ease: "easeOut"}} viewport={{once: true, amount: 0.2}}>
      ...
      >
        ...
        {products.map((product) => (
          <div key={product._id} className="lg:w-[400px]! w-[300px]! ">
            <div className="lg:w-[400px] lg:h-[530px] w-[300px] h-[400px] overflow-hidden">
              <img
            ...
            <div className="flex gap-[10px] mt-[16px]! mb-[16px] ">
              {product.slides.map((slide, index) =>
                index < 3 ? (
                  <div key={slide} className="w-[45px] h-[55px] shrink-0 ">
                    <img
                      src={slide}
                      alt=""
            ...
            <div className="text-[#333f48] text-[14px] ">{product.name}</div>
