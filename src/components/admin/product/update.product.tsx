  ...
  useEffect(() => {
    if (isOpen && product) {
      const thumbFile: UploadFile = {
        ...
        name: product.thumbnail || "thumbnail",
        fileName: product.thumbnail || "",
        status: "done",
        url: product.thumbnail || "",
      };

      const slideFiles: UploadFile[] = (product.slides || []).map(
        (url: string, index: number) => ({
          uid: `slide-${product._id}-${index}`,
          name: url,
          fileName: url,
          status: "done",
          url: url,
  ...
  const onFinish = async () => {
    ...
    try {
      ...
      const payload: ProductFormValues = {
        ...
        category:
          values.category ||
          product.category?.map((c: { _id: string }) => c._id) ||
          [],
        brand: values.brand || product.brand?._id || "",
        supplier: values.supplier || product.supplier?._id || "",
        priceSell: values.priceSell ?? product.priceSell ?? 0,
        costPrice: values.costPrice ?? product.costPrice ?? 0,
        material: values.material,
        weight: values.weight,
        tags: values.tags || product.tags || [],
        gender: values.gender || product.gender || [],
        thumbnail: fileListThumb[0]?.url,
        slides: fileListSlides.map((x: UploadFile) => x.url || ""),
        is: {
          active: values.isActive ?? product.is?.active ?? true,
          featured: values.isFeatured ?? product.is?.featured ?? false,
          new: values.isNew ?? product.is?.new ?? true,
          bestSeller: values.isBestSeller ?? product.is?.bestSeller ?? false,
          topRated: product.is?.topRated ?? false,
          discounted: isDiscounted,
        },
        sale: isDiscounted
          ? {
              discountType: values.saleDiscountType || "percentage",
              discountValue: values.saleDiscountValue ?? 0,
              startDate: values.startDate || product.sale?.startDate || "",
              endDate: values.endDate || product.sale?.endDate || "",
            }
          : {
      ...
      const response = await productService.updateProduct(product._id, payload);
          ...
          <Row gutter={16}>
            <Col span={6}>
              <Form.Item
                ...
                initialValue={product?.is?.active}
              >
                <Switch />
              </Form.Item>
            </Col>
            ...
            <Col span={6}>
              <Form.Item
                ...
                name="isFeatured"
                valuePropName="checked"
                initialValue={product?.is?.featured}
              >
                <Switch />
              </Form.Item>
            </Col>
            <Col span={6}>
              <Form.Item
                ...
                name="isBestSeller"
                valuePropName="checked"
                initialValue={product?.is?.bestSeller}
              >
                <Switch />
              </Form.Item>
            </Col>
