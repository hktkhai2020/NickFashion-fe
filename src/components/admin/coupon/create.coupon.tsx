...
const CreateCoupon = (props: {
  ...
  const onFinish = async () => {
    ...
    try {
      ...
      message.error(err?.response?.data?.message || "Tạo mã giảm giá thất bại");
    } finally {
      setLoading(false);
    }
  };
    ...
    >
      ...
      >
        ...
        <Row gutter={16}>
          ...
          <Col span={8}>
            <Form.Item
              label={couponType === "percentage" ? "Phần trăm (%)" : "Số tiền (VND)"}
              name="value"
        ...
        <Row gutter={16}>
          <Col span={8}>
            <Form.Item label="Đơn hàng tối thiểu (VND)" name="minOrderAmount">
              <InputNumber
                min={0}
                style={{ width: "100%" }}
          ...
          <Col span={8}>
            <Form.Item label="Mỗi user dùng tối đa" name="usageLimitPerUser">
              <InputNumber
                min={1}
                style={{ width: "100%" }}
                placeholder="1"
        ...
        <Row gutter={16}>
          <Col span={12}>
            <Form.Item label="Phạm vi" required>
              ...
              >
                <Select.Option value="public">Công khai (ai cũng dùng được)</Select.Option>
                <Select.Option value="private">Cá nhân (chỉ user được chỉ định)</Select.Option>
              </Select>
            </Form.Item>
          </Col>
          <Col span={12}>
        ...
        <Form.Item label="Ghi chú" name="note">
          <Input.TextArea placeholder="Ghi chú nội bộ (không hiển thị với user)" rows={2} />
        </Form.Item>
      </Form>
    </Modal>
  );
